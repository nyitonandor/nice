...
public class FixedByWeightByTotalComputationMethod : BasePlugin, IShippingRateComputationMethod
{
    ...
    // Custom implementation of GetFixedRateAsync using Fan Courier rates
    public async Task<decimal?> GetFixedRateAsync(GetShippingOptionRequest request)
    {
        ...
        // Same helper as in GetShippingOptionsAsync
        var totalWeight = await _shippingService.GetTotalWeightAsync(request, ignoreFreeShippedItems: true);

        // Your bands: adjust as needed
        var bands = new (decimal MaxWeight, decimal Price)[]
        {
        (1m, 16.74m),
        (3m, 18.29m),
        (5m, 20.14m),
        ...
        var match = bands.FirstOrDefault(b => totalWeight <= b.MaxWeight);
        if (match == default)
            return null; // above max supported weight
